...
import { RoleUser } from 'src/app/application/models/roleUser';
...
export class RoleUserComponent implements OnInit {
  ...
  url = apiRoutes.roles_user;
  singleData: RoleUser;
  data: PaginateResponse = new PaginateResponse(new RoleUser);
  ...
  async buildForm() {
    this.form = await this.formService.getForm(formIds.roleUserId);
