...
import { PermissionUser } from 'src/app/application/models/permissionUser';
...
export class PermissionUserComponent implements OnInit {
  ...
  url = apiRoutes.permissions_user;
  singleData: PermissionUser;
  data: PaginateResponse = new PaginateResponse(new PermissionUser);
  ...
  async buildForm() {
    this.form = await this.formService.getForm(formIds.permissionUserId);
  ...
  onDeleteAttempt(id: number) {
    if (!this.singleData) {
      this.singleData = new PermissionUser();
    }
